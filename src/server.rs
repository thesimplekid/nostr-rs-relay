...
async fn handle_web_request(
    ...
    match (
            ...
            if let Some(media_types) = accept_header {
                if let Ok(mt_str) = media_types.to_str() {
                    if mt_str.contains("application/nostr+json") {
                        ...
                        let rinfo = RelayInfo::new(settings.info, settings.pay_to_relay);
        ...
        ("/lnbits", false) => {
            ...
            if let Err(e) = payment_tx.send(PaymentMessage::InvoiceUpdate(callback)) {
...
"#,
        ...
        // TODO:
        // Add another route where user can check if theyre admitted
        // later balance
        (_, _) => {
